...
import {ValidationResponse, ValidationError, isNullOrWhiteSpace} from './utils'
...
const retreiveTxData = async (
  payloadData: Types.IVerifiedDataLegacy | Types.IVerifiedDataOnChain,
  web3Provider: string,
...
const validateOnChainProperties = (subject: string, payloadData: Types.VerifiedData, logs: TDecodedLog[]): ValidationError[] => {
  const errors: ValidationError[] = []
...
): Promise<ValidationResponse<Types.IVerifiablePresentation>> => {
