...
const validateOnChainProperties = (subject: string, payloadData: TVerifiedData, logs: TDecodedLog[]): TVerificationError[] => {
  const errors: TVerificationError[] = []
...
): Promise<{kind: 'invalid'; errors: TVerificationError[]} | {kind: 'validated'; data: IVerifiablePresentation; logs: TDecodedLog[]}> => {
  if (!responseData) {
    return {
      kind: 'invalid',
        ...
        switch (d.proof.data.version) {
          ...
          case DataVersions.onChain:
            try {
              ...
              errors.push(...validateOnChainProperties(d.credentialSubject.subject, d.proof.data, retreiveTxDataOutput.logs))
            } catch (err) {
              errors.push({
...
export const getOnChainCredentialProof = (
  ...
  const proof = formatMerkleProofForShare(bloomMerkleTree.getProof(EthU.toBuffer(HashingLogic.hashMessage(target.attesterSig))))
  return {
...
export const sortByKeys = (obj: {[i: string]: any}) => {
