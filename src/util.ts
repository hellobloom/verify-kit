...
export const sortObject = <T>(object: any): T => {
  let sortedObj: any = {};
  const keys = R.sortBy((key: string) => {
    return key;
  }, Object.keys(object));
...
export const verifyProof = (data: IVerifiedData): boolean => {
  ...
  const targetNode = toBuffer(
    HashingLogic.hashMessage(data.target.signedAttestation)
  );
...
export const verifyOffChainDataIntegrity = (
  ...
  if (!verifyProof(data)) {
    errors.push({
      key: "proof",
      error:
        "The provided 'proof' is invalid for the given 'signedAttestation' and 'rootHash'."
    });
  }
...
export const verifySender = (
  ...
  const signerEthAddress = HashingLogic.recoverHashSigner(
    toBuffer(responseData.packedData),
...
export const retreiveTxData = async (
  ...
  if (isNullOrWhiteSpace(txHash) || txHash === "0x") {
    errors.push({
      key: "tx",
      error: `tx is missing in payload for node with hash ${
        payloadData.layer2Hash
      }`
    });
  } else {
    try {
  ...
  const matchingTraitAttestedLogs =
    ...
    logs.find(
      l =>
        ...
        getDecodedLogValueByName(l, "dataHash") === payloadData.layer2Hash
    );
  if (!matchingTraitAttestedLogs) {
    errors.push({
      ...
      error:
        ...
        ` 'dataHash' of '${payloadData.layer2Hash}'.`
    });
    return errors;
  }
  ...
  await Promise.all(
    shareKitPayload.data.map(async d => {
      ...
      if (options.validateOnChain) {
        ...
        try {
          const retreiveTxDataOutput = await retreiveTxData(
            d,
          ...
          errors.push(
            ...validateOnChainProperties(responseData.subject, d, dTemp.logs)
          );
        } catch (err) {
          errors.push({
            key: "onChainValidationFailed",
            error: `Failed to validate on chain data integrity for node with hash ${
              d.layer2Hash
            }`
          });
        }
...
export const validateUntypedResponseData = async (
  ...
  if (!responseData) {
    return {
      errors: [
        {
          key: "missingResponseData",
          error: "Failed to validate falsey responseData"
        }
      ],
      data: []
    };
  ...
  const errors: TVerificationError[] = [];
  const fields: Array<keyof ResponseData> = [
    "token",
    "subject",
    "packedData",
    "signature"
