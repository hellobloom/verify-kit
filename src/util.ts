...
export const verifyOffChainDataIntegrity = (data: IVerifiedData): TVerificationError[] => {
  ...
  if (data.layer2Hash !== recoveredLayer2Hash) {
    errors.push({
      key: 'layer2Hash',
      error: "The provided 'layer2Hash' doesn't match the value" + " recovered for the given 'rootHash' and 'rootHashNonce'.",
    })
  }
  ...
  if (data.attester !== recoveredAttesterAddress) {
    errors.push({
      key: 'attester',
      error: "The provided 'attester' doesn't match the value" + " recovered for the target 'attestationNode' and 'signedAttestation'.",
    })
  }
...
export const retreiveTxData = async (payloadData: IVerifiedData, web3Provider: string): Promise<IRetrieveTxDataOutput> => {
...
const validateOnChainProperties = (subject: string, payloadData: IVerifiedData, logs: TDecodedLog[]): TVerificationError[] => {
  const errors: TVerificationError[] = []
