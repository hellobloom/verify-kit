...
test("Verifying layer2Hash, attester address, and merkle proof", () => {
  ...
  const merkleTreeComponents: IBloomMerkleTreeComponents = {
    rootHash:
      "0xfa0147ea749ba09f692162665de44b74801cfbeb16308aaf5788e87d0e1a09a1",
    dataNodes: [
      {
        attestationNode: {
          aux:
            "0x480f7971777eda1e6e6804f35435f5ae163623bf1404bda8f1018600f89d757f",
          data: {
            data: "eddiehedges@gmail.com",
            nonce:
              "5ee82099c52e30dc801131e12972fff1b8f90230dd268b04665c7385d959984b",
            version: "2.0.0"
          },
          link: {
            local:
              "0xdb81eabcbd65153a64b8c0e843e822c9c1f64bfbe5bf2481734a73288798b1b0",
            global:
              "0x97e9ad0b20ba0f0528efef244c4fe6ef11f5a6b0c7b2667064080cd6d81ca5ca",
            dataHash:
              "0xfd6a015fccf1a4140a40b939fc8755841324e3b3cc09cf02d325fb378aa72cbf",
            typeHash:
              "0xb98d14777c16823502b7c962bf576b00b6f9f23d12c2c8a3bc11699a2dcfe8da"
          },
          type: {
            type: "email",
            nonce:
              "fe11a2cb674207c0120e0058de3f5c60935ffa0abbb62c22c439ffa07c409022",
            provider: "Bloom"
          }
        },
        signedAttestation:
    ...
    layer2Hash:
      "0x6cca42a6266f647be85fba506fccc9925a995fee74fe08fe619c6a37cfbcb9ca",
    checksumSig:
    ...
    rootHashNonce:
      "0xa6a7d2b6d495bb12c0bb79d82bf5952ea8d5f14ceb948d5bf076b5b4c5f16517",
    signedRootHash:
  ...
  const signedAttestationHash = HashingLogic.hashMessage(
    merkleTreeComponents.dataNodes[0].signedAttestation
  );
  const merkleTree = HashingLogic.getMerkleTreeFromComponents(
    merkleTreeComponents
  );
  const proof = util.formatProofForShare(
    merkleTree.getProof(util.toBuffer(signedAttestationHash))
  );
...
test("Verify ResponseData that is structured incorrectly does not validate.", async () => {
  ...
  const undefinedResponseData = await util.validateUntypedResponseData(
    undefined,
    options
  );
  expect(undefinedResponseData.data).toHaveLength(0);
  expect(undefinedResponseData.errors).toHaveLength(1);
  expect(undefinedResponseData.errors.map(x => x.key)).toContain(
    "missingResponseData"
  );

  const nullResponseData = await util.validateUntypedResponseData(
    null,
    options
  );
  expect(nullResponseData.data).toHaveLength(0);
  expect(nullResponseData.errors).toHaveLength(1);
  expect(nullResponseData.errors.map(x => x.key)).toContain(
    "missingResponseData"
  );
  ...
  expect(emptyObject.errors.map(x => x.key)).toContain(
    "ResponseData.packedData"
  );
  expect(emptyObject.errors.map(x => x.key)).toContain(
