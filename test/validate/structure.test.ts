...
const legacyComponents = HashingLogic.getSignedMerkleTreeComponentsLegacy([emailAttestation, phoneAttestation], alicePrivkey)
...
test('Validation.validatePresentationSignature', () => {
  ...
  expect(
    Validation.validatePresentationSignature(
      HashingLogic.signHash(alicePrivkey, EthU.toBuffer(HashingLogic.hashMessage(HashingLogic.orderedStringify(presentationProof)))),
      presentation,
    ),
  ).toBeFalsy()
})
